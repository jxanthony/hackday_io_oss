...
class Hackday < ActiveRecord::Base
  ...
  def hack_at_index(num)
    queue(false).detect{ |hack| hack.presentation_index == num}
  end

  def join_queue(hack)
    hack.update_attribute(:presentation_index, queue(false).size + 1)
  end
  ...
  def move_down_in_queue(hack)
    return false if hack.presentation_index.nil? or hack.presentation_index + 1 > queue(false).size
  ...
  def add_break(position)
    ...
    breaktime.presentation_index = queue(false).size + 1
    breaktime.save!
  ...
  def bump_queue(index)
    ...
    queue(false).where("presentation_index > #{index}").each do |hack|
      former_index = hack.presentation_index
